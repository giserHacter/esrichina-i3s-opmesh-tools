...
class OSGBGeometry:
    ...
    def SetPosition(self,Verticals):
        ...
        for v in Verticals:
            value+=struct.pack('f',v)
        self.geometryBin=self.geometryBin[0:byteOffset]+value+self.geometryBin[byteOffset+len(Verticals)*4:]
    def SetPosition_jit(self,Verticals):
        byteOffset=8
        # value=struct.pack('f',X)+struct.pack('f',Y)+struct.pack('f',Z)
        value=bytes()
        for v in Verticals:
            for j in v:
                value+=struct.pack('f',j)
        self.geometryBin=self.geometryBin[0:byteOffset]+value+self.geometryBin[byteOffset+len(Verticals)*12:]
    # 根据索引设置位置
    # index：索引
    # num表示顶点数目
    # by xupf 删除几何点的保存算法
